...
from src.features import (
    ...
    compute_embeddings,
    run_etl,
    parse_skills,  # ✅ added (needed to compute overlap in graph-only tab)
)
...
def _record_view_and_show(user_label: str, mapping: Dict[str, str], job_id: str):
    ...
    _ = record_view(uid, jid)  # increments popularity + logs view
    return _format_job_details(jid, uid=uid, just_viewed=True)


# ----------------------------
# Tabs logic (Hybrid/Search/Explain/Popularity)
# ----------------------------
def overview(
    user_label: str,
...
def skill_graph_insights(user_label: str, mapping: Dict[str, str], top_k: int):
    ...
    # Always take job skills from dataset (users/jobs data)
    df["job_skills"] = df["job_id"].astype(str).apply(
        lambda jid: jobs.loc[jid]["skills"] if jid in jobs.index else ""
    )
    df["user_skills"] = str(user_sk)

    def _overlap(sk: str) -> str:
        j_set = set(parse_skills(str(sk)))
        inter = sorted(u_set.intersection(j_set))
        return ", ".join(inter)

    df["overlap_skills"] = df["job_skills"].apply(_overlap)
